...
export const BackgroundLines = ({
  ...
  return (
    <div
      className={cn(
        "h-screen w-full bg-gradient-to-b from-gray-100 to-gray-50 relative",
        className
      )}
    >
...
const SVG = ({
  ...
  // Create more paths by transforming existing ones
  const allPaths = [
    ...paths,
    ...paths.map(path => path.replace(/M720/g, 'M520')), // Left side
    ...paths.map(path => path.replace(/M720/g, 'M920')), // Right side
  ];
      ...
      {allPaths.map((path, idx) => (
        <motion.path
          ...
          transition={{
            ...
            repeat: Infinity,
            repeatType: "loop",
            delay: Math.floor(Math.random() * 5), // Reduced delay for more frequent animations
            repeatDelay: Math.floor(Math.random() * 3), // Reduced repeat delay
          }}
        />
      ))}
    </motion.svg>
